...
#define I_PRED_DATA IF_ISS_2.pred_data
    ...
    switch (I_OP) {
          case OP_FP_L_D:
            ...
            /*** Operando 2 ***/
            //Andreu: No fa res perque el op2 es per a ALU o Store
            LB[s].Q2 = MARCA_NULA;
        ...
        case OP_FP_S_D:
            ...
            /* INSERTAR CÓDIGO */
            SB[s].ocupado = SI;
            // I_OP -> macro que es refereix a la instrucció
            SB[s].OP = I_OP;
            // Marc: rob -> marca del registro
            SB[s].rob = b;  //Andreu: Preguntar

            /*** Operando 1 (en Rint) ***/
            /* INSERTAR CÓDIGO */
            if (Rint[I_S1].rob == MARCA_NULA) {//Llig valor
                // V1 -> valor, Q1 -> marca d'operador si en té
                SB[s].V1 = Rint[I_S1].valor;    
                SB[s].Q1 = MARCA_NULA;
            } // Si ha completat la operació 
            else if (RB[Rint[I_S1].rob].completado) {//Llig RB
                // Posa el resultat de la operació que s'ha fet en el operador corresponent
                SB[s].V1 = RB[Rint[I_S1].rob].valor;
                SB[s].Q1 = MARCA_NULA;
            } 
            // Si està ocupat l'operador
            ...
            /*** Operando 2 (en Rfp) ***/
            /* INSERTAR CÓDIGO */
            //Com es una ALU o store, si te oper2.
            if (Rfp[I_S2].rob == MARCA_NULA){
               // V2 -> valor, Q2 -> marca d'operador si en té
            ...
            RB[b].dest = s;
             ...
             // Marc: No reserva per al registre destí, ja que és un store, no escriu en registres (?)
        ...
        case OP_FP_SUB_D:
            ...
            RS[s].ocupado = SI;
            RS[s].OP = I_OP;
            RS[s].rob = b;  //Andreu: Preguntar

            /*** Operando 1 (en Rfp) ***/
            /* INSERTAR CÓDIGO */
            if (Rfp[I_S1].rob == MARCA_NULA) {//Llig valor
                RS[s].V1 = Rfp[I_S1].valor;
                RS[s].Q1 = MARCA_NULA;
            } else if (RB[Rfp[I_S1].rob].completado) {//Llig RB
