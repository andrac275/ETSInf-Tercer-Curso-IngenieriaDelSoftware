                   ...
                   int me, int np)
{
  ...
  MPI_Status status;
  MPI_Request solicitut; /*Andreu: per al req del que no espera*/
  int acabar; /*Andreu: variable per saber si han acabat els altres processos/workers */
  ...
  if (me == 0) {
    ...
    for ( rows_done = 0 ; rows_done < h ; rows_done++ ) {
      /* Andreu: Iniciar recepcio no bloquejant de una fila calculada per qualsevol proces */
      MPI_Irecv(B, w, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &solicitut);
      //Andreu: Comprobar si els procesos ens han tornat algo
      MPI_Test(&solicitut, &acabar, &status);
      /*Andreu: Mientras no se ha recibido nada y siguienteFila < filasTotales*/
      while(!acabar && next_row < h){
          /*Andreu: Compute the row. Es lo mateix que fan els treballadors */
          for ( j = 0 ; j < w ; j++ ) {
            z0 = (x1 + ix*j) + (y1 + iy*next_row)*I;
            ni = newton(z0, tol, maxiter);
            if ( ni > max ) max = ni;
            A(next_row,j) = ni; //Andreu
          }
          /*Andreu: siguiente fila = siguiente fila + 1*/
          next_row++;
          /*Andreu: filas hecas = filas hechas + 1*/
          rows_done++;
          /*Andreu: Comprobar de nou si els processos han acabat*/
          MPI_Test(&solicitut, &acabar, &status);
      } 
      /*Andreu: Si no han acabat encara els processos, esperar de manera bloquejant*/
      if(!acabar){MPI_Wait(&solicitut,&status);}
