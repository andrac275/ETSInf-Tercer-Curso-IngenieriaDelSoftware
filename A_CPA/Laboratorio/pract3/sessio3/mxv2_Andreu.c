...
int main(int argc, char *argv[])
{
  ...
  if (me == 0) {
    ...
    /*Andreu: Comente estes linies que les substituire per les directives de comunicacions*/
  ...
  /* Andreu: La linea 157 sustituye a la 149 y 153; la 158 sustituye a la 150 y 154*/
  ...
  for (iter = 1; iter <= num_iter; iter++) {
    ...
    //Andreu: Sustitueix send de 196 i receive de 184. Acumula perque es el que  fa 185 i 186
    //rep de molts i acumula per aixo el reduce.
    MPI_Reduce(xloc2,x,n,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
    
    //Andreu: Acumula. Sustitueix a les linies 177 178 179 180
    if (me == 0) {
        for (i = 0; i < n; i++){
          x[i] += v[i];
        }
    }
    //Andreu: Sustitueix send de 193 i receive de 197. Fa Scatter perque el root
    //divideix el vector entre els altres procesos. Aixo ho fa el root (linea 177)
    //en el codi base.
    MPI_Scatter(x,nb,MPI_DOUBLE,xloc1,nb,MPI_DOUBLE,0,MPI_COMM_WORLD);
...
#define ABS(a) ((a) >= 0 ? (a) : -(a))
  ...
  //Andreu: Comentades perque se sustitueixen en la 238
  // if (me == 0) {
  //   for (proc = 1; proc < num_procs; proc++) {
  ...
  //Andreu: Sustitucio de lo de dalt. Acumula el resultat final en 'aux'
  MPI_Reduce(&norma,&aux,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);

  t = MPI_Wtime() - t;

  if (me == 0) {
    norma = aux;  //Andreu: Copiar on toca per que ho mostre
